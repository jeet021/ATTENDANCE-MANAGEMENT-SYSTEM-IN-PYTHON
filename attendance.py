...
class Attendance:
    def __init__(self):
        ...
        title_of_this_program = tk.Label(
            self.root, text="Attendance Management System", font=('sans serif', 10), fg='blue')
        ...
        username_login_entry = ttk.Entry(
            self.root, textvariable=self.username_for_login_var)
        ...
        password_login_entry = ttk.Entry(
            self.root, textvariable=self.password_for_login_var, show="*")
        ...
        def login():
            self.loggedin(self.username_for_login_var.get(),
                          self.password_for_login_var.get())
        ...
        try:
            self.connection = mysql.connector.connect(
                host="localhost", password="", username="root", database="pythonproject")
        except:
    ...
    def signup(self):
        ...
        username_for_signup_label = tk.Label(
            signup_interface, text="Define Username")
        username_for_signup_label.place(x=105, y=40)
        username_entry_for_signup = ttk.Entry(
            signup_interface, textvariable=self.username_for_signup_var)
        username_entry_for_signup.place(x=90, y=60)
        # -------------lables for password and entry-----#
        password_for_signup_label = tk.Label(
            signup_interface, text="Define Password")
        password_for_signup_label.place(x=105, y=90)
        password_entry_for_signup = ttk.Entry(
            signup_interface, textvariable=self.password_for_signup_var, show="*")
        password_entry_for_signup.place(x=90, y=110)
        # ----callback function----#

        def createaccount():
            self.createaccountwithus(
                username_entry_for_signup.get(), password_entry_for_signup.get())
        # -----create account button------#
        create_account_button = ttk.Button(
            signup_interface, text="Create Account", command=createaccount)
        ...
        if username and password:
            if username.isalpha() == True:
                ...
                try:
                    cursor.execute(query)
                    messagebox.showinfo(
                        "Account Created", "Account Created Sucess,Proceed towards login")
                    self.connection.commit()
                except:
                    messagebox.showerror(
                        "Username already in use", "This username is already in use choose another username")
            else:
                messagebox.showerror(
                    "Username Error", "Username must be alphabet")
        else:
    ...
    def login_sucess(self, username):
        ...
        date_label = tk.Label(login_sucess_interface, text="{}".format(
            todays_date), font=("sans serif", 10))
        date_label.place(x=340, y=0)
        # -----you are now logged in as ----#
        logged_in_as_label = tk.Label(
            login_sucess_interface, text=f"You are now logged in as {username}", font=("sans serif", 11))
        logged_in_as_label.place(x=135, y=20)
        # ------select divisions-----#
        select_division = tk.Label(
            login_sucess_interface, text="Select Division")
        select_division.place(x=200, y=50)
        # ------divisions combobox-------#
        divisions_combobox = ttk.Combobox(
            login_sucess_interface, state="readonly", width=30)
        ...
        def getdetails():
            ...
            for enrollment, name in self.data:
                records_label = ttk.Label(
                    login_sucess_interface, text=f"{enrollment} {name}")
        ...
        def update():
            try:
                ...
                for details in details_with_status:
                    ...
                    messagebox.showinfo(
                        "Sucess", "Attendance Updated ++Sucessfully")
            except:
                messagebox.showerror(
                    "Select Student", "Please Select Student to update attendance")
        ...
        def change_pass():
            ...
            old_password_label = tk.Label(
                change_pas_interface, text="Old Password")
            ...
            new_password_label = tk.Label(
                change_pas_interface, text="New Password")
            ...
            def change():
                ...
                if old_pass != current_password:
                    messagebox.showerror(
                        "Password Did not match", "Password Didnot Matched check again")
                    ...
                    try:
                        ...
                        messagebox.showinfo(
                            "Update Sucess", "Password Updated Successfully")
                        change_pas_interface.destroy()
                    except:
                        messagebox.showerror(
                            "Error", "Error updating Password")
            # -----update password----#
            update_password_button = ttk.Button(
                change_pas_interface, text="Change", command=change)
        ...
        def add_student():
            ...
            enrollment_entry = ttk.Entry(
                add_student_window, textvariable=enrollment_var)
            ...
            select_division_label_combo = tk.Label(
                add_student_window, text="Select Division")
            select_division_label_combo.place(x=130, y=130)
            # -----division combobox----#
            select_division_combobox = ttk.Combobox(
                add_student_window, state="readonly", width=18)
            select_division_combobox['values'] = (
                'CEA', 'CEB', 'CEC', 'CED', 'CEE')
            ...
            def add_new_student():
                ...
                if len(enrollment) == 0 or len(name) == 0 or len(division) == 0:
                    messagebox.showwarning(
                        "No Empty Fields", "Please Fill the details")
                else:
                    try:
                        cursor = self.connection.cursor()
                        ...
                        self.connection.commit()
                        messagebox.showinfo(
                            "Sucess", "Student's Details Added")
                    except:
                        messagebox.showwarning(
                            "Student Already exits", "Student's Details Already Exits")
            # ----add data----#
            add_student_btn = ttk.Button(
                add_student_window, text="Add Student", command=add_new_student)
            add_student_btn.place(x=128, y=180)
            add_student_window.mainloop()
        # ------date entry------#
        date_entry = tkcalendar.DateEntry(
            login_sucess_interface, width=12, background='darkblue', foreground='white', borderwidth=2)
        date_entry.place(x=190, y=110)
        # -----student details-----#
        student_details_button = ttk.Button(
            login_sucess_interface, text="Get Students Details", command=getdetails)
        student_details_button.place(x=10, y=110)
        # -----clear data-------#
        clear_data_button = ttk.Button(
            login_sucess_interface, text="Clear Data", command=cleardata)
        clear_data_button.place(x=360, y=110)
        # ----change password button-----#
        change_password_button = ttk.Button(
            login_sucess_interface, text="Change Password", command=change_pass)
        change_password_button.place(x=330, y=140)
        # ----update attendance button-----#
        update_attendance = ttk.Button(
            login_sucess_interface, text="Update Attendance", command=update)
        update_attendance.place(x=180, y=400)
        # -----add student----#
        add_student_button = ttk.Button(
            login_sucess_interface, text="Add Student", command=add_student)
